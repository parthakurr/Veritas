...
import { Dumbbell, TrendingUp, Flame, LayoutDashboard, User, LogOut, Sparkles } from 'lucide-react';
...
export const Navbar: React.FC = () => {
  ...
  return (
    <nav className="sticky top-4 z-40 w-full max-w-6xl mx-auto px-4 sm:px-6">
      <div className="glass-panel rounded-full px-4 sm:px-6 py-2.5 flex items-center justify-between border border-slate-200/80 dark:border-slate-800 shadow-lg shadow-slate-200/50 dark:shadow-none backdrop-blur-xl bg-white/90 dark:bg-slate-900/90 transition-colors">
        ...
        <div className="flex items-center space-x-3">
          <Link href="/dashboard" className="flex items-center space-x-2 group">
            ...
            <span className="font-mono font-extrabold text-base tracking-tight text-slate-900 dark:text-white">
          ...
          {isAuthenticated && user ? (
            <div className="relative">
              <button
                ...
                className="flex items-center space-x-2 bg-slate-100 hover:bg-slate-200/70 border border-slate-200 rounded-full pl-2 pr-3 py-1 text-xs text-slate-800 transition"
              >
                {user.avatar ? (
                ...
                <span className="font-semibold max-w-[100px] truncate">{user.name.split(' ')[0]}</span>
              </button>

              {/* User Dropdown Menu */}
              {showDropdown && (
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-2xl p-2 border border-slate-200 shadow-2xl z-50 text-xs space-y-1">
                  <Link
                    href="/dashboard"
                    onClick={() => setShowDropdown(false)}
                    className="flex items-center space-x-2 px-3 py-2 rounded-xl text-slate-700 hover:text-slate-900 hover:bg-slate-100 transition font-medium"
                  >
                  ...
                  <button
                    ...
                    className="w-full text-left flex items-center space-x-2 px-3 py-2 rounded-xl text-rose-600 hover:bg-rose-50 transition font-medium"
                  >
            ...
            <button
              ...
              className="px-4 py-1.5 rounded-full bg-[#E03E2D] hover:bg-[#C93323] text-white font-extrabold text-xs transition shadow-md shadow-rose-500/20 flex items-center space-x-1.5"
            >
